...
function handleSubmit(event) {
  ...
  radioButtons.forEach((radioButton, index) => {
    // boucle sur chaque élément dans radioButtons                        responses[0]   responses[1] ...
    if (radioButton.value === responses[index]) {
      // compare la valeur de la case sélectionnée/réponse (a, b, c, d) et les réponses (  [   "c"     ,      "a"       , "b", "a", "c"]; )
      results.push(true);
    } else {
      results.push(false);
    }
...
function showResults(results) {
  ...
  console.log(
    "Resultats de results.filter(el => el === false): " + resultatsErrones
  );
...
function addColors(results) {
  results.forEach((response, index) => {
    if (results[index]) {
      // test si la valeur est vrai ou faux
